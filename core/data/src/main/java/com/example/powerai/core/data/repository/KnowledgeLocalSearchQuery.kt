...
data class KnowledgeLocalSearchQuery(
    val raw: String,
...
object KnowledgeLocalSearchQueryBuilder {
    fun buildOrNull(query: String): KnowledgeLocalSearchQuery? {
        ...
        // Allow single CJK char queries like "鐢?.
        if (normalized.length < 2 && !(hasCjk && normalized.length == 1)) return null

        // If query contains CJK, drop IME latin residue (e.g. "鍙樺帇qi" -> "鍙樺帇").
        if (hasCjk) {
